...
public class CollectionGoal : MonoBehaviour
{
    ...
    public static CollectionGoal CreateCollectionGoal(GameObject parent, PieceType pieceType, int number)
    {
        GameObject goalObject = new GameObject("CollectionGoal");
        goalObject.transform.SetParent(parent.transform);
        
        CollectionGoal collectionGoal = goalObject.AddComponent<CollectionGoal>();
        collectionGoal.prefabToCollect = Piece.LoadPrefab(pieceType).GetComponent<GamePiece>();
        collectionGoal.numberToCollect = number;
